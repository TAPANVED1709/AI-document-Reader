...
public class TimelineController : ControllerBase
{
    ...
    private static string Title(MedicalReport r, List<LabResult> results) => results.Select(x => x.SectionName).FirstOrDefault(x => !string.IsNullOrWhiteSpace(x)) ?? (r.OriginalFileName.Contains("cbc", StringComparison.OrdinalIgnoreCase) ? "Complete Blood Count" : r.OriginalFileName.Contains("liver", StringComparison.OrdinalIgnoreCase) ? "Liver Function Test" : r.OriginalFileName.Contains("thyroid", StringComparison.OrdinalIgnoreCase) ? "Thyroid Profile" : "Laboratory Report");
