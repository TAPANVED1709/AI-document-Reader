...
public class TrendsController : ControllerBase
{
    ...
    public async Task<IActionResult> Compare([FromBody] CompareRequest request, CancellationToken ct)
    {
        ...
        foreach (var id in selected.Select(r => r.MedicalReportId).Distinct())
            if (await _authorization.CanViewReportAsync(User, id, ct)) authorized.Add(id);
        selected = selected.Where(r => authorized.Contains(r.MedicalReportId)).ToList();
        var conflicting = LabResultTrust.ConflictingDemographicRows(selected);
        var points = selected.Select(r => ToPoint(r, conflicting.Contains(r.Id))).ToList();
