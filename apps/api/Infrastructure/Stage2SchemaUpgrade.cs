...
public static class Stage2SchemaUpgrade
{
    public static async Task ApplyAsync(DocumentDbContext db)
    {
        ...
        if (db.Database.IsSqlite())
        {
            ...
            try
            {
                ...
                if (await TableExistsAsync(db, "LabResults"))
                {
                    ...
                    await db.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS ProcessingJobs (Id TEXT PRIMARY KEY NOT NULL, ReportId TEXT NOT NULL UNIQUE, Status TEXT NOT NULL, AttemptCount INTEGER NOT NULL DEFAULT 0, MaxAttempts INTEGER NOT NULL DEFAULT 3, CreatedAt TEXT NOT NULL, QueuedAt TEXT NULL, StartedAt TEXT NULL, CompletedAt TEXT NULL, NextRetryAt TEXT NULL, HeartbeatAt TEXT NULL, LastErrorCode TEXT NULL, LastErrorSafeMessage TEXT NULL, ProcessingVersion TEXT NOT NULL, WorkerId TEXT NULL, FOREIGN KEY (ReportId) REFERENCES MedicalReports(Id) ON DELETE CASCADE)");
                    await db.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS IngestionIdempotencyRecords (Id TEXT PRIMARY KEY NOT NULL, OrganizationId TEXT NOT NULL, Key TEXT NOT NULL, ReportId TEXT NOT NULL, JobId TEXT NOT NULL, CreatedAt TEXT NOT NULL, UNIQUE(OrganizationId, Key))");
                }
            }
        ...
        else if (db.Database.IsSqlServer())
        {
            ...
            await db.Database.ExecuteSqlRawAsync("IF OBJECT_ID('ProcessingJobs', 'U') IS NULL CREATE TABLE ProcessingJobs (Id uniqueidentifier NOT NULL PRIMARY KEY, ReportId uniqueidentifier NOT NULL UNIQUE, Status nvarchar(30) NOT NULL, AttemptCount int NOT NULL DEFAULT 0, MaxAttempts int NOT NULL DEFAULT 3, CreatedAt datetimeoffset NOT NULL, QueuedAt datetimeoffset NULL, StartedAt datetimeoffset NULL, CompletedAt datetimeoffset NULL, NextRetryAt datetimeoffset NULL, HeartbeatAt datetimeoffset NULL, LastErrorCode nvarchar(80) NULL, LastErrorSafeMessage nvarchar(500) NULL, ProcessingVersion nvarchar(40) NOT NULL, WorkerId nvarchar(100) NULL, CONSTRAINT FK_ProcessingJobs_MedicalReports FOREIGN KEY (ReportId) REFERENCES MedicalReports(Id) ON DELETE CASCADE)");
            await db.Database.ExecuteSqlRawAsync("IF OBJECT_ID('IngestionIdempotencyRecords', 'U') IS NULL CREATE TABLE IngestionIdempotencyRecords (Id uniqueidentifier NOT NULL PRIMARY KEY, OrganizationId uniqueidentifier NOT NULL, [Key] nvarchar(200) NOT NULL, ReportId uniqueidentifier NOT NULL, JobId uniqueidentifier NOT NULL, CreatedAt datetimeoffset NOT NULL, CONSTRAINT UQ_IngestionIdempotency_Organization_Key UNIQUE (OrganizationId, [Key]))");
        }
    }
