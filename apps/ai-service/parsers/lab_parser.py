...
class LabRowParser(BaseParser):
    VALUE = r"(?<![A-Za-z0-9])(?:[<>]=?|[≤≥])?\s*[+-]?\d+(?:\.\d+)?|(?:Non-?Reactive|Reactive|Negative|Positive|Trace|Nil|Absent|Present)"
    UNIT = r"(?:gm?/d[Ll]|mg/d[Ll]|mcg/d[Ll]|ug/d[Ll]|mmol/[Ll]|mEq/[Ll]|meq/[Ll]|IU/[Ll]|U/[Ll]|[µu]IU/m[Ll]|mIU/[Ll]|10[³3\^�]*/[uµ]L|x10[³3\^�]*/[uµ]L|pg/m[Ll]|ng/m[Ll]|[µu]g/d[Ll]|%|mg/[Ll]|ug/[Ll]|f[Ll]|m[Ll]/min|ratio)"
    FLAGS = {"h":"H","high":"HIGH","l":"L","low":"LOW","n":"N","normal":"NORMAL","a":"A","abnormal":"ABNORMAL","*":"*"}
    SECTIONS = {"complete blood count":"Complete Blood Count","cbc":"Complete Blood Count","liver function test":"Liver Function Test","lft":"Liver Function Test","kidney function test":"Kidney Function Test","kft":"Kidney Function Test","renal function test":"Renal Function Test","lipid profile":"Lipid Profile","thyroid profile":"Thyroid Profile","diabetes profile":"Diabetes Profile","iron studies":"Iron Studies","electrolytes":"Electrolytes","urine examination":"Urine Examination"}
        ...
        for page in pages:
            ...
            for index, (line, tokens) in enumerate(rows):
                ...
                if key in self.SECTIONS:
                    section = self.SECTIONS[key]; continue
                if self._ignored(line): continue
                item = self._parse_row(line, page.page_number, section, tokens)
                if item:
                ...
                if self._looks_like_name(line) and index + 1 < len(rows):
                    ...
                    item = self._parse_row(block, page.page_number, section, block_tokens)
    ...
    def _ignored(self, line):
        ...
        if low.startswith(("method:", "reference range", "biological reference", "observed value", "test name", "investigation", "parameter")): return True
    ...
    def _parse_row(self, line: str, page: int, section: str | None, tokens) -> Optional[LabResultItem]:
        ...
        ref = self.references.parse(after_unit if after_unit else None)
